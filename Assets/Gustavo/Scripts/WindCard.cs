...
[CreateAssetMenu(fileName = "WindCard", menuName = "Scriptable Objects/Cards/WindWall")]
public class WindCard : Card
{
    ...
    public override void Use(GameObject user)
    {
        ...
        var proj = projObj.GetComponent<WindProjectile>();
        if (proj != null)
        {
            proj.Initialize(damage, projectileSpeed, projectileLifetime, pushForce, stunDuration, user);

            Quaternion rotation = cam.transform.rotation;
            rotation *= Quaternion.AngleAxis(-90, Vector3.right);

            proj.GetComponent<VisualEffect>().SetVector3("_Angle", rotation.eulerAngles);
        }
        else
        {
