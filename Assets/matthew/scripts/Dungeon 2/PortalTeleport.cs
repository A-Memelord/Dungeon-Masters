...
public class PortalTeleport : MonoBehaviour
{
   ...
   void OnTriggerEnter(Collider other)
   {
       ...
       // --- Position ---
       Vector3 localPos = portalSurface.InverseTransformPoint(other.transform.position);
       localPos.x = -localPos.x;
       localPos.z = -localPos.z;
       Vector3 newPos = exitSurface.TransformPoint(localPos);
       newPos += exitNormal * exitOffset;
       ...
       other.transform.SetPositionAndRotation(newPos, Quaternion.Euler(0f, exitRot.eulerAngles.y, 0f));
    ...
    //         if (preserveVelocity)
    //         {
    //             float speed = pm.moveSpeed.magnitude;
    //             pm.moveSpeed = Vector3.down * speed;
    //         }
    //    }
   }
}
